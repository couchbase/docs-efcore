...
. Finally, `DbContext.SaveChanges()` is called and the entity's modified properties are persisted to the database.
...
`SaveChanges` is a powerful model for persisting changes to the database, but has some limitations.
The main limitation is that each entity must be tracked and when `SaveChanges` is called, each modified or added entity will be persisted to the database.
...
More information on the differences between SaveChanges and `ExecuteDelete` and `ExecuteDelete` can be found in the https://learn.microsoft.com/en-us/ef/core/saving/[main EF Core documentation].


== Adding Data

Use the `DbSet<TEntity>.Add` method to add new instances of your entity classes. 
The data will be inserted into the database when you call `DbContext.SaveChanges()`:
...
[NOTE] 
EF Core Couchbase DB Provider GA 1.0 uses the https://docs.couchbase.com/dotnet-sdk/current/howtos/kv-operations.html[Couchbase Key/Value store] and not {sqlpp} for CRUD operations. 
