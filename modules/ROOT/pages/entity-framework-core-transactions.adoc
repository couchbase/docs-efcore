...
cluster (see xref:entity-framework-core-configuration.adoc#one-context-spanning-multiple-buckets[One context spanning multiple buckets]).
...
just-committed document -- KV reads aren't governed by the `RequestPlus` query-scan-consistency
option (that only affects N1QL queries), so there's a brief window before a point read reflects the
commit. If you need to read back a value you just wrote inside a transaction, poll briefly or
re-query rather than assuming an immediate KV read is guaranteed consistent.
...
An `IsolationLevel` overload also exists
(`BeginCouchbaseTransactionAsync(durabilityLevel, isolationLevel, cancellationToken)`); it's
informational only for Couchbase and doesn't change transaction behavior.
...
per cluster instead -- see xref:entity-framework-core-configuration.adoc#multiple-clusters[Multiple clusters]).
