...
sequences and client-side GUID generation -- see xref:entity-framework-core-sequences.adoc[Sequences and generated values]
for `UseSequence`, `[CouchbaseSequence]`, `UseGuid()`, and `UseGuidString()`. For everything else,
refer to the https://learn.microsoft.com/en-us/ef/core/modeling/generated-properties?tabs=data-annotations[EF Core documentation].
...
in <<Inheritance>> (including navigations declared only on a derived type) and on many-to-many
<<many-to-many,skip navigations>> below.
...
provider's primary way of modeling nested data -- see xref:entity-framework-core-limitations.adoc[Limitations]
for why (document-database nested data generally needs to be an owned type rather than a
separate, independently-queryable entity).
...
converter -- the provider calls the converter for `null` values on owned properties exactly like
it does for non-owned ones:
