...
More information on the differences between SaveChanges and `ExecuteDelete` and `ExecuteDelete` can be found in the main EF Core https://learn.microsoft.com/en-us/ef/core/saving/[documentation].
...
[NOTE] 
EF Core Couchbase DB Provider GA 1.0 uses the https://docs.couchbase.com/dotnet-sdk/current/howtos/kv-operations.html[Couchbase Key/Value store] and not SQL++ for CRUD operations. This may change in later releases.
...
[NOTE] 
Eager fetching via `Include` and/or `ThenInclude` is not supported in EF Core Couchbase DB Provider GA 1.
