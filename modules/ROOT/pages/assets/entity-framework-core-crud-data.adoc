...
In many scenarios, your program needs to query some data from the database, perform some modification on it, and save those modifications back; this is sometimes referred to as a https://en.wikipedia.org/wiki/Unit_of_work["Unit of Work"]. For example, let's assume that you have a set of Blogs, and you'd like to change the Url property of one of them.

[NOTE] 
The reasons for calling `DbContext.Dispose` in this manner are not the same in a RDBMS backed EFCore Provider as they are with the EFCore Couchbase DB Provider. 
The Couchbase Provider caches and reuses socket connections the entirety of the applications lifetime, so calling Dispose is effectively a `NOOP`.
...
[CAUTION] 
`ExecuteUpdate` and `ExecuteDelete` are only minimally supported in this developer preview, for experimental usage only.
