...
The xref:server:n1ql:n1ql-language-reference/metafun.adoc#meta[META function] is supported in the EF Core Couchbase DB Provider version 1.0.
Hhowever, lamda expression is not supported.
This means you will need to use the `FromSqlRaw` method to execute the query.
...
[TIP]
In addition to META, other {sqlpp} functions not yet supported in the EF Core Couchbase DB Provider may be executed in a similar way.
