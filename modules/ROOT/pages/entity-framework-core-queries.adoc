...
The following aggregate operators are supported by the GA:
...
Other operators may or may not be supported in the GA.

== SQL queries

=== SqlRaw

`DbContext.SqlRaw` is not implemented as of the EF Core Couchbase DB Provider GA because it depends on ADO.NET parameters which are minimally supported in the preview.

[NOTE] 
`DbContext.FromSql` will throw a `NotImplementedException` in EF Core Couchbase DB Provider GA 1.
...
=== META Support
The https://docs.couchbase.com/server/current/n1ql/n1ql-language-reference/metafun.html#meta[META function] is supported in the EF Core Couchbase DB Provider GA 1.0, however, lamda expression is not supported. This means you will need to use the `FromSqlRaw` method to execute the query.
